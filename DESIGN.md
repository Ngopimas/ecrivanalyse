...
|------|----------|-----------|
| 2026-07-04 | **Pages documentaires** : `/ecrivanalyse` (la présentation de la pratique par Ivan, cinq chapitres, articles 129-134) et `/editions` (l'intro ÉOK 14, les livres Omar Kaczmar 123-125, le côté Œuvres Komplètes 127 et 797 « Faire une quinte de tout »). Extraites par gen-content.py vers `src/data/legacy-pages.json` ; liées depuis /projet (« Pour aller plus loin ») ; les 39 pages fantômes lient le recueil vers /editions ; la 404 redirige leurs anciens ids. Seul le contact (118, 136) reste écarté. | 797 est la présentation du recueil que citent les fantômes ; 129-134 est le manifeste d'Ivan dans ses mots (à côté de la voix éditoriale de /projet). Demande de Romain. |
| 2026-07-04 | **Mémoire des anciennes adresses** : la page 404 redirige les URL SPIP (`spip.php?articleN` -> quinte ou texte, même id ; `rubriqueN` -> collection via la carte id->slug reconstruite depuis les fils d'Ariane des articles, `rubriques.json` étant une autre taxonomie ; `auteurN` -> /projet ; `id_article=N` des forums couvert). **La photographie de 190** (« Mise au Point par M. D. », seule page illustrée du corpus, vignette 210x139 seule rescapée du crawl) revient sous sa quinte, champs `image`/`image_caption`. **Cartes OG des textes** (titre + extrait italique + « texte · année »). **Recherche** : filtre quintes/textes (filtre Pagefind `type`). **Archive** : le rail des années suit le défilement (IntersectionObserver). | Vingt ans de liens et de résultats de recherche pointent vers les adresses SPIP ; une archive ne casse pas ses liens. Le reste : retours de la passe « 3, 4, 6 » de Romain. |
| 2026-07-04 | **La quinte d'une page hybride se lit dans la prose** : même corps, même graisse, à l'endroit exact où l'original la plaçait (marqueur `<quinte>` dans `prose`, position par défaut : la fin). Le traitement ajusté à la largeur reste réservé aux quintes seules. Format Markdown/MDX évalué et écarté pour l'instant : `lines` doit rester une donnée structurée (l'écriture mot à mot de la landing, les cartes OG, la recherche et le fit s'en servent) ; le marqueur donne la souplesse de position sans changer de format. La prose sans quinte existe déjà : ce sont les textes. | Sur la page d'origine de 154, la quinte achève la dernière phrase de la prose (« ...tout à la fois : ») dans le même corps ; la grossir la sortait du récit. Retour de Romain. |
| 2026-07-04 | **Le temps de la rencontre** : entre le recul vers le mur et la plongée, un battement (~560 ms) où la quinte choisie s'allume (ocre, scale 1.3) pendant qu'un voile de papier (un seul élément, opacité seule, halo radial centré sur le titre) fait reculer la masse. **La quinte la plus récente** est toujours dans l'échantillon de la plongée et placée au centre visuel du mur ; un visiteur récurrent pour qui elle est nouvelle atterrit dessus à l'arrivée (localStorage `ea-latest`), une seule fois. Lede de l'archive simplifié en « 3 047 quintes et 97 textes, archivés de 2009 à 2026 ». | Retour de Romain : la quinte sélectionnée n'était pas lisible dans le mur (le .lit s'appliquait dans la même frame que le départ de la caméra). Le battement rend le choix visible et porte la métaphore (une voix sort de la masse) ; le ciblage de la nouveauté sert les visiteurs fidèles sans figer le hasard. |
